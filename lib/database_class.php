...
class DataBase {
	...
	//мои
	public function getSense($table_name, $fields, $max = true) {
		if (!$max) {
			$data = $this->select($table_name, array("MIN(`$fields`)"));
			return $data[0]["MIN(`$fields`)"];
		} 
		$data = $this->select($table_name, array("MAX('".$fields."')"));
		return $data[0]["MAX(`$fields`)"];
	}	
	
	public function getSenseFromInterval($table_name, $where, $value_start, $value_stop) {
		$data = $this->select($table_name, $where = "$where BETWEEN .$value_start AND .$value_stop");
	...
	public function getFieldOnID($table_name, $id, $field_out) {
		...
		return $this->getFeild($table_name, $field_out, "id", $id);
	}
	...
	public function serFieldOnID($table_name, $id, $field, $value) {
		if (!$this->existsID($table_name, $id)) return false;
